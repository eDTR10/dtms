...
const SignatureSettings = () => {
  ...
  const [testBaked,     setTestBaked]     = useState<string|null>(null); // data-url preview
  ...
  const handleBakePreview = async () => {
    ...
    setTestBaking(true);
    try {
      // Rebuild stamp composite (same logic as buildStampCanvas in SignDocument)
      const W  = 1000;
      // Direct ratio from stamp box dimensions — avoids page-aspect skew
      ...
      const drawStampText = () => {
        ...
        if (displayName) {
          ...
          sCtx.fillText(displayName, (txtLeft / 100) * W, ty);
        }
        if (position) {
          sCtx.font      = `${posFs}px sans-serif`;
          sCtx.fillStyle = "#2563EB";
          sCtx.fillText(position, (txtLeft / 100) * W, ty + nameFs * 1.35);
        }
      };
      ...
      // Composite onto PDF canvas copy
      ...
      // Stamp position in canvas pixels
  ...
  // Mouse drag handler — tracks both X and Y inside the stamp box
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
  ...
  const handleClear = () => {
    ["sig_password","sig_displayName","sig_position","sig_image_data",
     ...
     "sig_img_top","sig_img_left","sig_txt_top","sig_txt_left"].forEach(k => localStorage.removeItem(k));
  ...
  return (
    <UserLayout title="Signature Settings" subtitle="Configure your personal digital signing credentials">
      ...
      <div className="flex flex-col gap-5">
        ...
        {/* ── Two-column grid ── left: credentials · right: designer + preview */}
        ...
        <div className="bg-card border border-border rounded-2xl p-6 sm:p-4 flex flex-col gap-5">
          ...
          {/* Stamp designer (draggable) + sliders side-by-side */}
              ...
              <div className="flex gap-4 items-start sm:flex-col">
                ...
                <div className="shrink-0">
                  ...
                  >
                    ...
                    <div
                      className="absolute text-center cursor-grab active:cursor-grabbing"
                      style={{ top: `${txtTop}%`, left: `${txtLeft}%`, transform: "translate(-50%, 0)", whiteSpace: "nowrap" }}
                ...
                <div className="flex flex-col gap-4 flex-1 min-w-0">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input type="range" min={8} max={24} step={1} value={textSize}
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input type="range" min={50} max={300} step={10} value={imgWidth}
                      onChange={e => setImgWidth(Number(e.target.value))}
                      className="w-full accent-primary" />
                    <div className="flex justify-between text-[11px] text-muted-foreground">
                      <span>50px</span><span>300px</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input type="range" min={140} max={420} step={10} value={stampWidth}
                      onChange={e => setStampWidth(Number(e.target.value))}
                      className="w-full accent-primary" />
                    <div className="flex justify-between text-[11px] text-muted-foreground">
                      <span>140pt</span><span>420pt</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input type="range" min={50} max={220} step={5} value={stampHeight}
                      onChange={e => setStampHeight(Number(e.target.value))}
                      className="w-full accent-primary" />
                    <div className="flex justify-between text-[11px] text-muted-foreground">
                      <span>50pt</span><span>220pt</span>
                    </div>
                  </div>

                </div>{/* end sliders */}
              </div>{/* end stamp box + sliders row */}
            </div>
          )}
              ...
              <div className="flex flex-col gap-2">
                ...
                {testBaked && (
                  <div className="flex flex-col gap-1">
                    <p className="text-xs text-green-600 font-medium flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Stamp preview — this is roughly how it will look in the signed PDF</p>
                ...
                {!testBaked && (
                  <div ref={testContainerRef} className="relative w-full border border-border rounded overflow-hidden bg-gray-100">
                      ...
                      <div className="absolute inset-0 cursor-crosshair" style={{ zIndex: 10 }}
                        ...
                        {testHoverPx && (
                          <div className="absolute border-2 border-blue-400 rounded pointer-events-none overflow-hidden"
                            style={{ left: testHoverPx.left, top: testHoverPx.top,
                            ...
                            {signImagePreview && (
                              <img src={signImagePreview} alt="sig" className="absolute object-contain pointer-events-none"
                                style={{ top: `${imgTop}%`, left: `${imgLeft}%`, transform: "translate(-50%,0)",
                                         width: imgWidth * testRenderScale, maxWidth: "90%" }} />
                            )}
                            <div className="absolute text-center"
                              style={{ top: `${txtTop}%`, left: `${txtLeft}%`, transform: "translate(-50%,0)", whiteSpace: "nowrap" }}>
                              <p className="font-bold text-blue-900 truncate" style={{ fontSize: Math.max(7, textSize * testRenderScale) }}>{displayName}</p>
                              {position && <p className="text-blue-700 truncate" style={{ fontSize: Math.max(6, (textSize-2) * testRenderScale) }}>{position}</p>}
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    {!testPlacing && testPlaced && (() => {
                      ...
                      const cssH    = testBoxH * testRenderScale;
                      return (
                        <div className="absolute border-2 border-blue-500 rounded overflow-hidden select-none"
                          style={{ left: cssLeft, top: cssTop, width: cssW, height: cssH,
                                   background: "rgba(59,130,246,0.10)", zIndex: 5, pointerEvents: "all" }}>
                          {/* Grip bar */}
                          <div className="absolute top-0 left-0 right-0 flex items-center justify-center bg-blue-600/80 cursor-move z-10"
                            style={{ height: Math.max(10, 14 * testRenderScale) }}
                            onMouseDown={e => { e.preventDefault(); e.stopPropagation();
                              testDraggingStamp.current = { startX: e.clientX, startY: e.clientY, origX: testSigX, origY: testSigY }; }}>
                            <span style={{ fontSize: Math.max(7, 9 * testRenderScale), color: "white", opacity: 0.9, letterSpacing: 2 }}>⣿</span>
                          </div>
                          {/* Content */}
                          <div className="absolute inset-0 overflow-hidden pointer-events-none"
                            style={{ paddingTop: Math.max(10, 14 * testRenderScale) }}>
                            {signImagePreview && (
                          ...
                          {/* Resize handle */}
                          <div className="absolute bottom-0 right-0 flex items-center justify-center bg-blue-600 cursor-se-resize z-10"
                            style={{ width: Math.max(10, 14 * testRenderScale), height: Math.max(10, 14 * testRenderScale),
                                     borderTopLeftRadius: 3, color: "white", fontSize: Math.max(8, 10 * testRenderScale) }}
                            onMouseDown={e => { e.preventDefault(); e.stopPropagation();
                              testResizingStamp.current = { startX: e.clientX, startY: e.clientY,
                                origW: testBoxW, origH: testBoxH, origY: testSigY }; }}>
                            ⌟</div>
                        </div>
                      );
                    })()}
                ...
                {testPlaced && !testPlacing && !testBaked && (
                  <p className="text-xs text-muted-foreground flex items-center gap-1">
                    <span className="font-mono">⣿</span> drag bar to move &nbsp;·&nbsp; <span className="font-mono">⌟</span> corner to resize &nbsp;·&nbsp; click <span className="font-semibold">Bake Preview</span> to render
                  </p>
                )}
              </div>
            )}
