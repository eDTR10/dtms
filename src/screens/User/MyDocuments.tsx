...
import { FileText, Send, Eye, Clock, CheckCircle2, Search, Download, RefreshCw, Pencil, Trash2, AlertTriangle, Loader2, ChevronLeft, ChevronRight, ChevronDown, GitBranch, Plus, X as XIcon, Link2, Link2Off, PenLine, Printer, Archive, ArchiveRestore, FileSymlink } from "lucide-react";
...
const statusLabel = (doc: Document, currentUserId?: number): string => {
  ...
  if (doc.status === "For Signing") {
    if (isSignatory && hasSigned) return `Incomplete (${signed}/${total})`;
...
const MyDocuments = () => {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // SINGLE SOURCE OF TRUTH: getDisplayStatus
  // Used by BOTH the table rows AND the stat card counts so they always match.
  // ─────────────────────────────────────────────────────────────────────────────
  const getDisplayStatus = (d: Document): string => {
    ...
    const total = signers.length;
    const signed = signers.filter(s => s.status === "signed").length;
    // A "signatory" here means: in the signatories list AND not just a viewer
    const isSignatory = sigs.some(s => s.user_id === user?.id && s.role !== "viewer");
    const hasSigned = sigs.some(s => s.user_id === user?.id && s.status === "signed" && s.role !== "viewer");
    const allSigned = total > 0 && signed === total;

    if (isViewer) return mySig?.status === "viewed" ? "Viewed" : "Viewing";
    if (d.status === "Pending") return "For Sending";
    if (d.status === "Rejected") return "Rejected";

    if (allSigned || d.status === "Completed") {
      // Owner always sees "Completed" — even if they also happen to be a signatory.
      // This prevents owner docs from leaking into the "Signed" filter tab.
      if (isOwner) return "Completed";
      // Non-owner signatory who signed → "Signed"
      if (isSignatory && hasSigned) return "Signed";
      // Non-owner, not a signatory (e.g. was a viewer but now all done) → "Completed"
      return "Completed";
    }

    if (d.status === "For Signing") {
      if (isSignatory && hasSigned) return "Incomplete";
      return "For Signing";
    }
  ...
  // toggleSelectAll and selectAllFiltered operate on `filtered` (current filter result)
  const toggleSelectAll = () => {
    const visibleTracks = paginated.map(d => d.tracknumber);
  ...
  const loadGoogleIdentityScript = () => {
    ...
    if (googleIdentityScriptPromiseRef.current) return googleIdentityScriptPromiseRef.current;
  ...
  const promptDriveFolderLink = async () => {
    ...
    return { folderId: extractDriveFolderId(result.value), folderLink: result.value };
  };
  ...
  const pickDownloadDirectory = async () => {
    ...
    if (!directoryPicker) throw new Error("Directory save is not supported in this browser.");
    const directoryHandle = await directoryPicker({ id: "dtms-download-all", mode: "readwrite", startIn: "downloads" });
    if (directoryHandle.requestPermission) {
      const permission = await directoryHandle.requestPermission({ mode: "readwrite" });
      if (permission === "denied") throw new Error("Directory access was denied.");
    }
  ...
  const buildMergedPdf = async (selectedDocs: Document[]) => {
    ...
    for (const doc of selectedDocs) {
      ...
      for (const file of files) {
        ...
        copiedPages.forEach(page => { mergedPdf.addPage(page); importedPageCount += 1; });
      }
    }
    if (importedPageCount === 0) throw new Error("No PDF pages were available to merge.");
  ...
  const printBlob = (blob: Blob) => new Promise<void>((resolve, reject) => {
    ...
    if (isEdgeBrowser()) {
      ...
      if (!printWindow) { URL.revokeObjectURL(blobUrl); reject(new Error("Failed to open print preview window.")); return; }
      window.setTimeout(() => {
        try {
          printWindow.focus(); printWindow.print();
          window.setTimeout(() => { printWindow.close(); URL.revokeObjectURL(blobUrl); resolve(); }, 1500);
    ...
    const cleanup = () => { URL.revokeObjectURL(blobUrl); iframe.remove(); };
    iframe.onload = () => {
      try {
        iframe.contentWindow?.focus(); iframe.contentWindow?.print();
        window.setTimeout(() => { cleanup(); resolve(); }, 1000);
      } catch (error) { cleanup(); reject(error); }
    };
    iframe.onerror = () => { cleanup(); reject(new Error("Failed to load printable document.")); };
    iframe.src = blobUrl;
  ...
  const handleDownloadSelected = async () => {
    ...
    try {
      ...
      const totalFiles = countDownloadFiles(selectedDocs);
      await Swal.fire({ icon: "success", title: "Download complete", text: `${totalFiles} file${totalFiles === 1 ? "" : "s"} downloaded successfully.`, confirmButtonText: "OK" });
    } catch (error) {
  ...
  const handleSendToDrive = async () => {
    ...
    try {
      ...
      const totalFiles = countDownloadFiles(selectedDocs);
      await Swal.fire({ icon: "success", title: "Drive upload complete", text: `${totalFiles} file${totalFiles === 1 ? "" : "s"} uploaded to Google Drive successfully.`, confirmButtonText: "OK" });
    } catch (error) {
      console.error("Drive upload failed", error);
      await Swal.fire({ icon: "error", title: "Drive upload failed", text: error instanceof Error ? error.message : "Failed to upload files to Google Drive.", confirmButtonText: "OK" });
    } finally {
  ...
  const handleChooseDownloadFolder = async () => {
    ...
    try { await pickDownloadDirectory(); } catch (error) { console.error("Failed to choose download folder", error); }
  };

  const batchDownloadProgressPercent = batchDownloadTotal > 0
    ? Math.min(100, Math.round((batchDownloadCompleted / batchDownloadTotal) * 100)) : 0;
  const driveUploadProgressPercent = driveUploadTotal > 0
    ? Math.min(100, Math.round((driveUploadCompleted / driveUploadTotal) * 100)) : 0;
  ...
  const handleResend = async () => {
    ...
    try {
      const payload = {
        ...
        signatories: resendDoc.signatories.map(s => ({ user_id: s.user_id, user_email: s.user_email, user_name: s.user_name, order: s.order })),
      };
  ...
  const openRouting = async (doc: Document) => {
    setRoutingDoc(doc);
    setRoutingSigs(
      (doc.signatories ?? []).sort((a, b) => a.order - b.order)
        .map(s => ({ user_id: s.user_id, user_email: s.user_email, user_name: s.user_name, order: s.order }))
    );
    setRoutingOffice(""); setRoutingSearch(""); setRoutingPage(0); setRoutingError(null);
    try {
      const [offices, users] = await Promise.all([officeApi.list(), userApi.signatories()]);
      setRoutingOffices(offices); setRoutingUsers(users);
    } catch (e) { console.error(e); }
  };

  const handleRoutingSave = async () => {
    if (!routingDoc) return;
    setRoutingSaving(true); setRoutingError(null);
    try {
  ...
  const openEdit = (doc: Document) => {
    setEditDoc(doc); setEditTitle(doc.title); setEditType(doc.type);
    setEditMsg(doc.message || ""); setEditFile(null); setEditError(null);
  };
  ...
  useEffect(() => {
    ...
    return () => { controller.abort(); if (tid !== null) clearTimeout(tid); };
  }, []);
  ...
  const toggleArchiveTrack = (tracknumber: string) => {
    setArchivedTracks(prev => prev.includes(tracknumber) ? prev.filter(t => t !== tracknumber) : [...prev, tracknumber]);
  ...
  return (
    <UserLayout title="My Documents" subtitle="Documents you created or are assigned to sign">
      ...
      {/* Quick stats — always count from all docs, not filtered */}
          ...
          <div className="relative shrink-0 sm:w-full">
            <div className="flex items-center gap-2 sm:flex-col sm:items-stretch">
              ...
              <div className="flex items-center gap-2 rounded-lg border border-border bg-background px-3 py-2 sm:w-full">
                ...
                <input type="date" value={dateToFilter} min={dateFromFilter || undefined} onChange={e => setDateToFilter(e.target.value)}
                  className="min-w-[140px] bg-transparent text-sm text-foreground [color-scheme:light] dark:[color-scheme:dark] focus:outline-none sm:min-w-0 sm:w-full" />
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {selectedTracks.length > 0 && (
            <div className="flex w-full flex-wrap items-center gap-2 rounded-lg border border-border bg-background px-3 py-1.5 md:w-auto">
              ...
              <button
                type="button" onClick={handleChooseDownloadFolder} disabled={!supportsDirectorySave()}
          ...
          <div className="flex flex-col gap-1.5">
            <div className="relative">
              <select value={officeSelection} onChange={e => handleOfficeSelect(e.target.value)}
            ...
            {selectedOffices.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {selectedOffices.map(officeValue => {
                  const officeLabel = officeOptions.find(o => o.value === officeValue)?.label ?? officeValue;
                  return (
                    <span key={officeValue} className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">
                      {officeLabel}
                      <button type="button" onClick={() => setSelectedOffices(prev => prev.filter(v => v !== officeValue))}
                        className="rounded-full text-primary/80 transition hover:text-primary" aria-label={`Remove office ${officeLabel}`}>
          ...
          <div className="flex flex-col gap-1.5">
            <div className="relative">
              <select value={projectSelection} onChange={e => handleProjectSelect(e.target.value)}
                className="appearance-none rounded-lg border border-border bg-background px-3 py-1.5 pr-8 text-sm text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 transition min-w-[180px]">
            ...
            {selectedProjects.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {selectedProjects.map(projectValue => {
                  const projectLabel = projectOptions.find(o => o.value === projectValue)?.label ?? projectValue;
                  return (
                    <span key={projectValue} className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">
                      {projectLabel}
                      <button type="button" onClick={() => setSelectedProjects(prev => prev.filter(v => v !== projectValue))}
                        className="rounded-full text-primary/80 transition hover:text-primary" aria-label={`Remove project ${projectLabel}`}>
                        <XIcon className="w-3 h-3" />
                      </button>
                    </span>
                  );
          ...
          <div className="flex flex-col gap-2 p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl animate-in fade-in slide-in-from-top-2">
            <div className="flex items-center gap-3 md:flex-col md:items-stretch md:gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2 md:grid md:grid-cols-2 md:gap-2">
                <button onClick={handleDownloadSelected} disabled={batchDownloading}
                  ...
                  {batchDownloading ? <><Loader2 className="w-4 h-4 animate-spin" /> Downloading...</> : <><Download className="w-4 h-4" /> Download All</>}
                </button>
                <button onClick={handleSendToDrive} disabled={driveUploading}
                  ...
                  {driveUploading ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending...</> : <><GoogleDriveIcon className="w-4 h-4" /> Send To Drive</>}
                </button>
                <button onClick={handlePrintSelected} disabled={batchPrinting}
                  ...
                  {batchPrinting ? <><Loader2 className="w-4 h-4 animate-spin" /> Printing...</> : <><Printer className="w-4 h-4" /> Print All</>}
                </button>
                <button onClick={() => navigate(`/dtms/sign/batch?tracks=${selectedTracks.join(",")}`)}
                ...
                <button onClick={() => setSelectedTracks([])}
              ...
              <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-3 py-2">
                <div className="flex items-center justify-between gap-3 text-xs font-medium text-emerald-700 dark:text-emerald-400">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="truncate">Downloading {batchDownloadCompleted} of {batchDownloadTotal} file{batchDownloadTotal === 1 ? "" : "s"}</span>
                  </div>
                ...
                <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-emerald-500/15">
                  <div className="h-full rounded-full bg-emerald-500 transition-[width] duration-300 ease-out" style={{ width: `${batchDownloadProgressPercent}%` }} />
                </div>
              </div>
            )}
            {driveUploading && (
              <div className="rounded-lg border border-sky-500/20 bg-sky-500/10 px-3 py-2">
                <div className="flex items-center justify-between gap-3 text-xs font-medium text-sky-700 dark:text-sky-400">
                  <div className="flex items-center gap-2 min-w-0">
                    <Loader2 className="w-3.5 h-3.5 animate-spin shrink-0" />
                    <span className="truncate">Uploading {driveUploadCompleted} of {driveUploadTotal} file{driveUploadTotal === 1 ? "" : "s"} to Google Drive</span>
                  </div>
                ...
                <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-sky-500/15">
                  <div className="h-full rounded-full bg-sky-500 transition-[width] duration-300 ease-out" style={{ width: `${driveUploadProgressPercent}%` }} />
                </div>
              </div>
            )}
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden min-h-[530px] flex flex-col">
        ...
        {loading ? (
          ...
          // Always render `paginated` — never bypass pagination
          paginated.map(doc => (
            <div
              key={doc.id}
            ...
            >
              ...
              <div className="flex items-center sm:items-end gap-1.5 w-full">
                <button onClick={() => navigate(`/dtms/sign/${doc.tracknumber}`)}
                  className="p-1.5 rounded-md text-muted-foreground hover:bg-accent hover:text-primary transition-colors" title="View">
                ...
                {doc.userID === user?.id && doc.status === "Pending" && (
                  <button onClick={() => openRouting(doc)}
                ...
                {doc.userID === user?.id && doc.status === "Rejected" && (
                  <button onClick={() => setResendDoc(doc)}
                ...
                <button onClick={() => toggleArchiveTrack(doc.tracknumber)}
                  ...
                  {isArchivedTrack(doc.tracknumber) ? <ArchiveRestore className="w-4 h-4" /> : <Archive className="w-4 h-4" />}
                </button>
                {doc.userID === user?.id && (
                  <button onClick={() => { setDeleteDoc(doc); setDeleteError(null); }}
      ...
      {/* Pagination — always shown when there are multiple pages */}
      {!loading && (
      ...
      {/* Edit Routing modal */}
      {routingDoc && (() => {
        const filteredUsers = routingUsers.filter(u => {
        ...
        const totalRoutingPages = Math.ceil(filteredUsers.length / PAGE_SIZE);
        const paged = filteredUsers.slice(routingPage * PAGE_SIZE, (routingPage + 1) * PAGE_SIZE);
        return (
            ...
            <div className="bg-card border border-border rounded-2xl shadow-xl w-full max-w-lg flex flex-col max-h-[90vh]">
                      ...
                      routingSigs.map((s, i) => {
                        ...
                        return (
                          <div key={s.user_id}>
                            ...
                            <div className={`flex items-center gap-3 rounded-lg px-4 py-2.5 ${isParallelWithAbove ? "bg-blue-500/5 border border-blue-500/20" : "bg-accent/50"}`}>
                  ...
                  {routingOffice && (
                    <>
                      <input type="text" placeholder="Search by name or position..."
                        value={routingSearch} onChange={e => { setRoutingSearch(e.target.value); setRoutingPage(0); }}
                          ...
                          <>
                            ...
                            {totalRoutingPages > 1 && (
                              <div className="flex items-center justify-between px-4 py-2 border-t border-border bg-accent/30">
                                <span className="text-xs text-muted-foreground">{routingPage * PAGE_SIZE + 1}–{Math.min((routingPage + 1) * PAGE_SIZE, filteredUsers.length)} of {filteredUsers.length}</span>
              ...
              <div className="flex gap-3 px-6 py-4 border-t border-border">
                ...
                <button onClick={handleRoutingSave} disabled={routingSaving || routingSigs.length === 0}
                  ...
                  {routingSaving ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending...</> : <><Send className="w-4 h-4" /> Save &amp; Send</>}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
            ...
            <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-3 mb-4">
              <p className="text-sm font-medium text-orange-600 dark:text-orange-400">Have you updated your document based on the feedback?</p>
            </div>
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => { setResendDoc(null); setResendError(null); }} disabled={resending}
                className="px-4 py-2 rounded-lg border border-border text-sm text-muted-foreground hover:bg-accent transition-colors disabled:opacity-50">Cancel</button>
              <button onClick={handleResend} disabled={resending}
                ...
                {resending ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending...</> : <><RefreshCw className="w-4 h-4" /> Yes, Re-send</>}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-2xl shadow-xl w-full max-w-lg flex flex-col max-h-[90vh]">
            <div className="flex items-center gap-3 px-6 pt-6 pb-4 border-b border-border">
              ...
              <button onClick={() => setEditDoc(null)} className="text-muted-foreground hover:text-foreground transition-colors text-lg leading-none">&times;</button>
            </div>
            <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-foreground">Replace PDF <span className="text-muted-foreground font-normal">(optional)</span></label>
                {editDoc.file_url && !editFile && (
                  <p className="text-xs text-muted-foreground">Current file: <span className="font-mono">{editDoc.file_url.split("/").pop()}</span></p>
                )}
                <label className="flex items-center gap-3 rounded-lg border border-dashed border-border bg-background px-4 py-3 cursor-pointer hover:border-amber-400/60 transition">
                  ...
                  <span className="text-sm truncate">
                    {editFile ? <span className="text-foreground">{editFile.name}</span> : <span className="text-muted-foreground">Click to select a new PDF file</span>}
                  </span>
                  <input type="file" accept="application/pdf" className="hidden" onChange={e => setEditFile(e.target.files?.[0] ?? null)} />
                </label>
              </div>
              {editError && (
                <div className="flex items-center gap-2 bg-destructive/10 border border-destructive/30 text-destructive text-sm rounded-lg px-3 py-2.5">
          ...
          <div className="bg-card border border-border rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="flex gap-3">
              <button onClick={() => { setDeleteDoc(null); setDeleteError(null); }} disabled={deleting}
                className="flex-1 py-2.5 rounded-lg border border-border text-sm text-muted-foreground hover:bg-accent transition disabled:opacity-50">Cancel</button>
